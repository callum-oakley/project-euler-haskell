...
chooseFrom :: [a] -> Int -> [[a]]
chooseFrom xS@(x:xs) n
    | n == 0 = [[]]
    | n >= length xS = [xS]
    | otherwise = map (x:) (chooseFrom xs (n - 1)) ++ chooseFrom xs n

coinDecomp :: Int -> Int -> Int
coinDecomp _ 0 = 1
coinDecomp maxCoin n = sum . map (\c -> cond (c > n) 0 (coinDecomp c (n - c)))
    $ dropWhile (/= maxCoin) [200,100,50,20,10,5,2,1]

collatzLengths :: Int -> Array Int Int
collatzLengths n = memo
    where memo = listArray (1,n) $ 1:[1 + len x | x <- [2..n]]
          len x = cond (x' <= n) (memo ! x') (1 + len x')
            where x' = cond (even x) (x `div` 2) (3*x + 1)

concatInts :: [Int] -> Int
concatInts = read . concat . map show
...
count :: (Eq a) => a -> [a] -> Int
count x = foldl (\acc y -> (cond (y == x) (acc + 1) acc)) 0

countAll :: (Ord a) => [a] -> Map.Map a Int
countAll xs = Map.fromList $ map (\x -> (x, count x xs)) $ nub xs

decToBin :: Int -> String
decToBin n = showIntAtBase 2 intToDigit n ""

digitFacSum :: Int -> Int
digitFacSum = sum . map (fac . digitToInt) . show
...
divisors :: Int -> [Int]
divisors n = sort $ foldr (\r acc ->
    cond (r `divides` n) (cond (r^2 == n) (r:acc) (r:(n `div` r):acc)) acc)
        [1,n] [2..floor . sqrt . fromIntegral $ n]

elemIndexArray :: (Ix i, Eq e) => e -> Array i e -> Maybe i
elemIndexArray x = fmap fst . find (\(i,e) -> e == x) . assocs

expandFrac :: Int -> Int -> [(Int, Int)]
expandFrac n d = iterate (\(_,r) -> (10*r `div` d, 10*r `mod` d))
    $ (n `div` d, n `mod` d)
...
filterByIndex p = map snd . filter (p . fst) . zip [0..]

filterEvenIndex :: ([a] -> [a])
filterEvenIndex = filterByIndex (\i -> mod i 2 == 0)

isAbundant :: Int -> Bool
isAbundant n = (sum . divisors $ n) > 2*n

isAmicable :: Int -> Bool
isAmicable n = (d . d $ n) == n && (not . isPerfect $ n)
...
isCircularPrime n =
    (and . map (not . (`elem` (show n))) $ ['2','4','5','6','8','0']) &&
    (and . map (isPrime . read) . rotations . show $ n)
...
lexPerm :: (Ord a) => [a] -> Int -> [a]
lexPerm xs n = build (blueprint (length xs - 1) n) xs
    where blueprint 0 _ = []
          blueprint k n = coef:(blueprint (k - 1) (n - (fac k)*coef))
            where coef = (last . takeWhile ((<= n) . (fac k *)) $ [0..])
          build [] s = s
          build (b:bs) s = next:(build bs (delete next s))
            where next = s !! b

lexPerms :: (Ord a) => [a] -> [[a]]
lexPerms xs = map (lexPerm xs) [0..(fac . length $ xs) - 1]

noOfDivisors :: Int -> Int
noOfDivisors = product . map (\x -> 1 + length x) . group . primeFactors

numLetterCount :: Int -> Int
numLetterCount = strLetterCount . show
    where strLetterCount (u:[]) = unitLengths u
          strLetterCount ('1':u:[]) = teenLengths u
          strLetterCount (t:u:[]) = tensLengths t + unitLengths u
          strLetterCount (h:'0':'0':[]) = unitLengths h + 7
          strLetterCount (h:'0':u:[]) = unitLengths h + 10 + unitLengths u
          strLetterCount (h:t:u:[]) = unitLengths h + 10 + strLetterCount [t,u]
          strLetterCount (t:'0':'0':'0':[]) = 11
          strLetterCount _ = 0

pascalRow :: (a -> a -> a) -> a -> [a] -> [a]
pascalRow op pad row = zipWith op ([pad] ++ row) (row ++ [pad])

palindromeInts :: [Int] -- could be made way more efficient with zipWith3
palindromeInts = filter isPalindrome [0..]

readSudoku :: String -> Sudoku
readSudoku = listArray ((0,0),(8,8)) . map digitToInt

recurringSegment :: (Eq a) => [a] -> [a]
recurringSegment = (\xs -> dropWhile (last xs /=) (init xs)) . head
    . filter (\xs -> last xs `elem` init xs) . drop 2 . inits

reduce :: (Int, Int) -> (Int, Int)
reduce (n, d) = (n `div` g, d `div` g)
    where g = gcd n d

reduceStupid :: (Int, Int) -> (Int, Int)
reduceStupid (n, d) = (cond (null . strip $ n) n (read . strip $ n),
                       cond (null . strip $ d) d (read . strip $ d))
    where strip = (\\ common) . show
          common = intersect (show n) (show d)

replicateEach :: Int -> [a] -> [a]
replicateEach n = foldr (\x acc -> replicate n x ++ acc) []
...
showSudoku = concatMap (\s -> s ++ "\n") . chunksOf 18
    . concatMap (\d -> [intToDigit d, ' ']) . elems

solveSudoku :: Sudoku -> [Sudoku]
solveSudoku s =
    cond (isJust maybeTarget)
    (concatMap solveSudoku . map 
        (\g -> s // [(target, g)]) . sudokuGuess s $ target) [s]
    where maybeTarget = elemIndexArray 0 s
          target = fromJust maybeTarget

solveSudokuNice :: String -> IO ()
solveSudokuNice = mapM_ (putStrLn . showSudoku) . solveSudoku . readSudoku

sudokuGuess :: Sudoku -> (Int, Int) -> [Int]
sudokuGuess s (x,y) = (\\) [1..9] . map ((!) s) $ (lines ++ square)
    where lines = concatMap (\k -> [(x,k),(k,y)]) [0..8]
          square = [(x `div` 3 * 3 + i, y `div` 3 * 3 + j) | 
                    i <- [0..2], j <- [0..2]]

triangleNums :: [Int]
triangleNums = map (\n -> n*(n + 1) `div` 2) [1..]

unitLengths :: Char -> Int
unitLengths = (Map.!) dict 
    where dict = Map.fromAscList [('0', 0),
                                  ('1', 3),
                                  ('2', 3),
                                  ('3', 5),
                                  ('4', 4),
...
teenLengths :: Char -> Int 
teenLengths = (Map.!) dict 
    where dict = Map.fromAscList [('0', 3),
                                  ('1', 6),
                                  ('2', 6),
                                  ('3', 8),
                                  ('4', 8),
...
tensLengths :: Char -> Int
tensLengths = (Map.!) dict 
    where dict = Map.fromAscList [('2', 6),
                                  ('3', 6),
                                  ('4', 5),
                                  ('5', 5),
                                  ('6', 5),
...
-- proceed with caution. it gets pretty messy down here...
-- should really move this data in to a new file...
